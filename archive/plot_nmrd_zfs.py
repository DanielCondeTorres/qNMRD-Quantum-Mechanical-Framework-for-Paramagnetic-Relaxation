...
B0_range = np.logspace(-4, 0, 30)  # De 0.0001 T (0.004 MHz) a 1 T (42 MHz)
freq_MHz = B0_range * gamma_I / (2 * np.pi * 1e6)

# Parámetros para igualar la gráfica (tau_c = 150 ps aprox, distancias reales)
r1_Mn = calc_r1(S=5/2, D_cm=0.00, r_A=2.8, tau_c_ps=200.0, B0_array=B0_range)
r1_Gd = calc_r1(S=7/2, D_cm=0.03, r_A=3.1, tau_c_ps=200.0, B0_array=B0_range)
r1_Fe = calc_r1(S=5/2, D_cm=0.06, r_A=2.8, tau_c_ps=200.0, B0_array=B0_range)

print("Frecuencia (MHz) | Mn(II) | Gd(III) | Fe(III)")
for i, f in enumerate(freq_MHz):
    if f < 0.1 or f > 10.0:
        print(f"{f:14.4f} | {r1_Mn[i]:6.2f} | {r1_Gd[i]:7.2f} | {r1_Fe[i]:7.2f}")
